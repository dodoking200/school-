...
export const getPermissionDescription = (permission: Permission): string => {
  const descriptions: Record<string, string> = {
    [USER_PERMISSIONS.MANAGE_ACADEMIC_YEARS]: 'Create, update, and delete academic years',
    ...
    [USER_PERMISSIONS.MANAGE_STUDENTS]: 'Create, update, and delete student records',
    [USER_PERMISSIONS.MANAGE_TEACHERS]: 'Create, update, and delete teacher records',
    ...
    [USER_PERMISSIONS.MANAGE_STUDENT_ATTENDANCE]: 'Record and manage student attendance',
    [USER_PERMISSIONS.MANAGE_STAFF_ATTENDANCE]: 'Record and manage staff attendance',
    [USER_PERMISSIONS.VIEW_ATTENDANCE_REPORTS]: 'View attendance reports and analytics',
    [USER_PERMISSIONS.MANAGE_BEHAVIORS]: 'Record and manage student behavior incidents',
    [USER_PERMISSIONS.VIEW_OWN_BEHAVIORS]: 'View personal behavior records',
    [USER_PERMISSIONS.MANAGE_TUITION_PAYMENTS]: 'Process and manage tuition payments',
    [USER_PERMISSIONS.VIEW_PAYMENT_REPORTS]: 'View payment reports and analytics',
    [USER_PERMISSIONS.MANAGE_NOTIFICATIONS]: 'Create and manage system notifications',
    ...
    [USER_PERMISSIONS.ATTENDANCE_MOBILE_APP]: 'Access attendance mobile application',
...
export const getFeaturePermissions = (feature: string): Permission[] => {
  const featurePermissions: Record<string, Permission[]> = {
    ...
    behaviors: [USER_PERMISSIONS.MANAGE_BEHAVIORS, USER_PERMISSIONS.VIEW_OWN_BEHAVIORS],
    payments: [USER_PERMISSIONS.MANAGE_TUITION_PAYMENTS, USER_PERMISSIONS.VIEW_PAYMENT_REPORTS],
    dashboard: [USER_PERMISSIONS.VIEW_DASHBOARD],
    settings: [USER_PERMISSIONS.MANAGE_PERMISSIONS, USER_PERMISSIONS.MANAGE_ROLES],
  };
...
/**
 * Validate if a set of permissions is valid
 */
export const validatePermissions = (permissions: Permission[]): boolean => {
...
export const hasFeatureAccess = (
  ...
  if (requireAll) {
    return featurePerms.every(permission => userPermissions.includes(permission));
  } else {
    return featurePerms.some(permission => userPermissions.includes(permission));
  }
};
